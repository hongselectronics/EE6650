...
def plot_return_loss(directories):
    ...
    categories = {
        ...
        'OFF': {'color': 'black', 'label': 'FPGA HWT-Free power on'},
        'ON': {'color': 'red', 'label': 'FPGA HWT-Free power off'}
    }
    ...
    for directory in directories:
        for file in os.listdir(directory):
            if file.endswith('.s1p') and ('FPGA' in file):
                ...
                f_start, f_stop = 4.9e9, 5.1e9

                # Convert frequency range to indices
                indices = np.where((network.f >= f_start) & (network.f <= f_stop))[0]
                if len(indices) == 0:
                    continue  # Skip this file if no frequencies are in the desired range

                # Slice the network for the desired frequency range using indices
                network = network[indices[0]:indices[-1]+1]
